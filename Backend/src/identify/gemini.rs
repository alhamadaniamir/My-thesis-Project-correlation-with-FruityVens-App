...
const PROMPT: &str = "Identify the fruit in this image. Reply with exactly one label from this list and nothing else: Apple, Mango Carabao, Indian Mango, Apple Mango, Dragon Fruit, Watermelon, Pineapple, Mangosteen, Banana, Avocado, Grapes, Durian, Pomelo, Pear, Lemon, Orange, Unknown. Never reply with the generic word Mango on its own. A fruit with its stem sunk in a well at the top and a dimple at the base is Apple. If the fruit is a mango, judge the silhouette and the skin color, not the size in frame, and pick one variety: Mango Carabao is long and slender with an S curve and a pointed beak at the tip, thin smooth skin, pale green when unripe and deep golden yellow when ripe; Indian Mango is flat and thin in profile with a hooked kidney like curve, waxy deep green skin, usually sold unripe; Apple Mango is a mango: round and plump, its skin covered in fine pale speckles, a red or orange blush fading smoothly into yellow, the stem sitting on a smooth unbroken shoulder with no well and no dimple at the base. If two varieties look equally likely, or the fruit is not on the list, reply Unknown. No punctuation, no explanation, no full sentence.";
