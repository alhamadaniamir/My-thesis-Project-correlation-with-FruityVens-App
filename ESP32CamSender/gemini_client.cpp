...
// Copy `raw` into `out` (size `out_size`) as Title Case so the returned
// fruit name has consistent capitalization.
void titleCaseCopy(const char* raw, char* out, size_t out_size) {
  ...
  for (size_t i = 0; raw[i] && j < out_size - 1; i++) {
    ...
    if (c == ' ' || c == '\t' || c == '-') {
      ...
      out[j++] = atWordStart ? toupper((unsigned char)c) : tolower((unsigned char)c);
      atWordStart = false;
    }
  }
  out[j] = '\0';
...
bool geminiIdentify(const uint8_t* jpg, size_t jpg_len, char* out_label, size_t out_size) {
  ...
  // Return whatever fruit Gemini reported, just tidied to Title Case.
  titleCaseCopy(raw_buf, out_label, out_size);
  Serial.print("Gemini: ");
